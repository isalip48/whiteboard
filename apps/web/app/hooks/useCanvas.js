...
export function useCanvas(socket) {
  ...
  // useState for tool settings — these DO need to trigger re-render (UI updates)
  ...
  const startDrawing = useCallback((canvasRef, e) => {
  ...
  const draw = useCallback((canvasRef, e) => {
    ...
    if (socket) {
      socket.emit('draw', { x0, y0, x1, y1, color: tool.color, size: tool.size });
    }
  ...
  return { tool, setTool, startDrawing, draw, stopDrawing, drawLine };
}